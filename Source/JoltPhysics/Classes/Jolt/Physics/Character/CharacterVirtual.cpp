...
CharacterVirtual::CharacterVirtual(const CharacterVirtualSettings *inSettings, RVec3Arg inPosition, QuatArg inRotation, uint64 inUserData, PhysicsSystem *inSystem, BodyID* inBodyID) :
//@ BASTIEN END
	...
	mUserData(inUserData)
{
	...
	if (inSettings->mInnerBodyShape != nullptr)
	{
		...
		if (inBodyID != nullptr)
		{
			body = mSystem->GetBodyInterface().CreateBodyWithID(*inBodyID, settings);
		}
		else
		{
			body = mSystem->GetBodyInterface().CreateBody(settings);
		}
		if (body != nullptr)
...
bool CharacterVirtual::SetShape(const Shape *inShape, float inOffset, float inMaxPenetrationDepth, const BroadPhaseLayerFilter &inBroadPhaseLayerFilter, const ObjectLayerFilter &inObjectLayerFilter, const BodyFilter &inBodyFilter, const ShapeFilter &inShapeFilter, TempAllocator &inAllocator)
//@ BASTIEN END
{
	...
	if (inShape != mShape && inShape != nullptr)
	{
		if (inMaxPenetrationDepth < FLT_MAX)
		{
			...
			Vec3 offsetPosition = mPosition + inOffset * mUp;
			GetContactsAtPosition(offsetPosition, mLinearVelocity.NormalizedOr(Vec3::sZero()), inShape, contacts, inBroadPhaseLayerFilter, inObjectLayerFilter, inBodyFilter, inShapeFilter);
			//@ BASTIEN END
