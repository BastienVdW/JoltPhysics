...
Character::Character(const CharacterSettings *inSettings, RVec3Arg inPosition, QuatArg inRotation, uint64 inUserData, PhysicsSystem *inSystem, BodyID* inBodyID) :
	...
	mLayer(inSettings->mLayer)
{
	...
	//@ BASTIEN ADD
	settings.mMotionQuality = inSettings->mMotionQuality;
	settings.mAllowSleeping = inSettings->mAllowSleeping;
